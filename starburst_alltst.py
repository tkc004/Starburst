...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 27 19:19:06 2022

SFR = Star Formation Rate
M_total = Total mass of galaxy
timestep = increment of timestep
@author: calum
"""
import numpy as np
...
'''Input file / Inputs. Also set-up and read in'''

times_spectra = np.arange(0.01e6, 50e6, 1e6)
times_steps = np.arange(0.00e6, 50e6, 0.1e6)
times_steps_log = np.log10(times_steps, where=times_steps > 0)

#Include when comparing with SB99 in this script
times_steps_SB99 = np.arange(0.01e6, 50e6, 0.1e6) 
hires_wave_grid_SB99 = np.genfromtxt('ifa_line_wave_grid.txt')

'''SFR is no longer an input option and instead will be applied through a SFR light-ratio weighting method post-processing'''
M_total = 1.0e6

IMF_exponents = [1.3, 2.3]
IMF_mass_limits = 0.1, 0.5, 120.

#Variable interpolation resolution factor, lower for speed up or higher for higher resolution interpolation
resolution_factor = 1
...
#spec_with_time_title = 'Evolution of Z=0.02 SEDs with rotation and VMS'
...
if POWR == False:
    hires_wave_grid = np.genfromtxt('ifa_line_wave_grid.txt')
    np.save('hires_wave_grid.npy', hires_wave_grid)
WN_powr_grid_file = 'galaxy/lejeune/PoWR-WN-Z020.txt'
WC_powr_grid_file = 'galaxy/lejeune/PoWR-WC-Z020.txt'
...
if Z =='supersol':
    ...
    if IMF_mass_limits[-1] > 300.:
        ...
        exit()
    if rot == True:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z20v40_buildtst_VMS.txt', skip_header=6)
        minimum_wr_mass=20.
        SBmodel_name = 'Z020_V40_sampling' #Z020_V40_sampling
    elif rot == False:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z20v00_buildtst_VMS.txt', skip_header=6)
        minimum_wr_mass=20.
        SBmodel_name = 'Z020_V00_sampling_FW' #comparing with no VMS plots    #SBmodel_name = 'Z014_V00_sampling'    #SBmodel_name = 'Z14_V00_sampling_FW_VMS_tracks' #Use tjis one!    #SBmodel_name = 'Z014_V00_M500_wm'    #SBmodel_name = 'Z014_V00'
    lowmass_params = np.genfromtxt('spec_params_lowmassm04_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_m04_reform_Aug.txt')# * 12
    #SBmodel_name = 'Z014_V00_sampling_Z020'
    lowmass_params = np.genfromtxt('spec_params_lowmassp00_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_p00_reform_Aug.txt')# * 12
    hires_params = np.genfromtxt('spec_params_ifa_line_p00.txt')
    hires_flux = np.genfromtxt('ifa_line_p00_reform.txt')
    hires_cont_flux = np.genfromtxt('ifa_cont_p00_reform.txt') #'''WRONG!!! '''
    minimum_wr_mass=20.
    WN_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WN_Z020_test.dat'
    WC_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WC_Z020_test.dat'
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z020_test.dat'
    if SPEC == 'FW':
        #spectra_grid_file = 'FW_SB_grid_Z020_30mod.txt'
        spectra_grid_file = 'FW_SB_grid_Z020_30mod_VMS.txt'
...
if Z == 'GAL':
    if rot == True:
        ...
        if IMF_mass_limits[-1] > 300.:
            ...
            exit()
        evo_tracks = np.genfromtxt('galaxy/tracks/Z0140v40_buildtst_VMS.txt', skip_header=6)
        minimum_wr_mass=20.
        #spectra_grid_file = 'FW_SB_gridSB99_GAL_500.txt'
        SBmodel_name = 'Z014_V40_sampling' #Z014_V40_sampling_FW
    elif rot == False:
        ...
        if IMF_mass_limits[-1] > 500.:
            ...
            exit()
        evo_tracks = np.genfromtxt('galaxy/tracks/Z0140v00_moremass_buildtst.txt', skip_header=6)
        #np.save('tracks_test.npy', evo_tracks)
        #evo_tracks = np.genfromtxt('galaxy/tracks/Z0140v00_buildtst_integer.txt', skip_header=6)
        spectra_grid_file = 'FW_SB_grid_GAL.txt'
        minimum_wr_mass=25.
        #SBmodel_name = 'Z14_V00_sampling_FW_VMS_tracks' #comparing with VMS plots
        SBmodel_name = 'Z14_V00_sampling_FW'#'Z14_V00_sampling_FW_VMS_tracks'   Z014_V00_sampling_24mod_WM #comparing with no VMS plots    #SBmodel_name = 'Z014_V00_sampling'    #SBmodel_name = 'Z14_V00_sampling_FW_VMS_tracks' #Use tjis one!    #SBmodel_name = 'Z014_V00_M500_wm'    #SBmodel_name = 'Z014_V00'
    lowmass_params = np.genfromtxt('spec_params_lowmassp00_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_p00_reform_Aug.txt')# * 12
    hires_params = np.genfromtxt('spec_params_ifa_line_p00.txt')
    hires_flux = np.genfromtxt('ifa_line_p00_reform.txt')
    hires_cont_flux = np.genfromtxt('ifa_cont_p00_reform.txt')
    WN_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WN_Z020_test.dat'
    WC_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WC_Z020_test.dat'
    WN_powr_grid_file = 'galaxy/lejeune/PoWR-WN-Z020.txt'
    WC_powr_grid_file = 'galaxy/lejeune/PoWR-WC-Z020.txt'
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z020_test.dat'
        #spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z020_lowsampling.dat'
        np.save('WMbasic_OB_Z020_test.npy', spectra_grid_file)
    if SPEC == 'FW':
        spectra_grid_file = 'FW_SB_grid_GAL.txt'
        np.save('FW_SB_grid_GAL.npy', spectra_grid_file)
        if IMF_mass_limits[-1] > 120.0:
            spectra_grid_file = 'FW_SB_grid_GAL_VMS.txt'
            np.save('FW_SB_grid_GAL_VMS.npy', spectra_grid_file)
    np.save('spec_params_lowmassp00.npy', lowmass_params)
    np.save('lcb97_p00_reform.npy', lowmass_flux)
    np.save('spec_params_ifa_line_p00.npy', hires_params)
    np.save('ifa_line_p00_reform_T.npy', np.transpose(hires_flux))
    np.save('ifa_cont_p00_reform.npy', hires_cont_flux)
    np.save('PoWR-WN-Z020.npy', WN_powr_grid_file)
    np.save('PoWR-WC-Z020.npy', WC_powr_grid_file)
...
if Z == 'LMC':
    ...
    if IMF_mass_limits[-1] > 300.:
        ...
        exit()
    if rot == True:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z006v40_buildtst_VMS.txt', skip_header=6)
        minimum_wr_mass=25.
        SBmodel_name = 'Z006_V40_sampling' #Z006_V40_sampling_FW Z006_V40_sampling_FW_VMS_fake
    elif rot == False:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z006v00_buildtst_VMS.txt', skip_header=6)
        minimum_wr_mass=25.
        SBmodel_name = 'Z006_V00_sampling' #'Z006_V00_sampling_FW_VMS' Z006_V00_sampling_FW
    lowmass_params = np.genfromtxt('spec_params_lowmassm04_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_m04_reform_Aug.txt')# * 12
    hires_params = np.genfromtxt('spec_params_ifa_line_m04.txt')
    hires_flux = np.genfromtxt('ifa_line_m04_reform.txt')
    hires_cont_flux = np.genfromtxt('ifa_cont_p00_reform.txt') #'''WRONG!!! '''
    WN_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WN_Z008_tst2.dat'
    WC_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WC_Z008_tst.dat'
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z008_tst.dat'
    if SPEC == 'FW':
        spectra_grid_file = 'FW_SB_grid_LMC.txt'
        if IMF_mass_limits[-1] > 120.0:
            spectra_grid_file = 'FW_SB_grid_LMC_VMS.txt'
...
if Z == 'SMC':
    ...
    if IMF_mass_limits[-1] > 120.:
        ...
        exit()
    #evo_tracks = np.genfromtxt('galaxy/tracks/Z0140v00_buildtst.txt', skip_header=6) #for comparing with current SB99
    if rot == True:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z002v40_buildtst.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z002_V40_sampling'
        if SPEC == 'FW':
            SBmodel_name = 'Z002_V40_sampling' #'Z002_V40_sampling_FW' Z002_V40_1Myr_sampling_FW_m07 Z002_V40_sampling_FW_m07
    elif rot == False:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z002v00_buildtst.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z002_V00_sampling'
        if SPEC == 'FW':
            #SBmodel_name = 'Z002_V00_sampling_FW_m07' #'Z002_V00_sampling_FW'Z002_V00_1Myr_sampling_FW_m07 
            SBmodel_name = 'Z002_V00_sampling' #for comparing with WMbasic
    lowmass_params = np.genfromtxt('spec_params_lowmassm07_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_m07_reform_Aug.txt')# * 12
    #SBmodel_name = 'Z004_002_V00'
    hires_params = np.genfromtxt('spec_params_ifa_line_m07.txt')
    hires_flux = np.genfromtxt('ifa_line_m07_reform.txt')
    hires_cont_flux = np.genfromtxt('ifa_cont_p00_reform.txt') #'''WRONG!!! '''
    minimum_wr_mass=84.
    WN_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WN_Z004_tst.dat'
    WC_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WC_Z004_tst.dat'
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z004_tst.dat'
    if SPEC == 'FW':
        spectra_grid_file = 'FW_SB_gridSB99_SMC.txt'
...
if Z == 'IZw18':
    ...
    if IMF_mass_limits[-1] > 120.:
        ...
        exit()
    if rot == True:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z0004v40_buildtst.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z0004_V40_sampling'
        if SPEC == 'FW':
            SBmodel_name = 'Z0004_V40_sampling' #'Z0004_V40_1Myr_sampling_FW' Z0004_V40_sampling_FW
    elif rot == False:
        evo_tracks = np.genfromtxt('galaxy/tracks/Z0004v00_buildtst.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z0004_V00_sampling'
        if SPEC == 'FW':
            SBmodel_name = 'Z0004_V00_sampling' #'Z0004_V00_1Myr_sampling_FW'  Z0004_V00_sampling_FW
    lowmass_params = np.genfromtxt('spec_params_lowmassm13_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_m13_reform_Aug.txt')# * 12
    hires_params = np.genfromtxt('spec_params_ifa_line_m13.txt')
    hires_flux = np.genfromtxt('ifa_line_m13_reform.txt')
    hires_cont_flux = np.genfromtxt('ifa_cont_p00_reform.txt') #'''WRONG!!! '''
    minimum_wr_mass=84.
    WN_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WN_Z001_tst.dat'
    WC_spectra_grid_file = 'galaxy/lejeune/CMFGEN_WC_Z001_tst.dat'
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z001_tst.dat'
    if SPEC == 'FW':
        spectra_grid_file = 'FW_SB_gridSB99_IZw18.txt'
...
if Z == 'Z0':
    if rot == True:
        ...
        if IMF_mass_limits[-1] > 120.:
            ...
            exit()
        evo_tracks = np.genfromtxt('galaxy/tracks/Z00v40_buildtst.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z00_V40_sampling'
        if SPEC == 'FW':
            SBmodel_name = 'Z00_V40_sampling' #Z00_V40_sampling_FW_nplanck
    elif rot == False:
        mass_grid = [300., 250., 120., 85., 60., 40., 30., 20., 15., 12., 9., 7., 5., 4., 3., 2.5, 2., 1.7]
        if IMF_mass_limits[-1] > 300.:
            print('Tracks do not exist at Z=0.0 above 300Msol')
            exit()
        evo_tracks = np.genfromtxt('galaxy/tracks/Z00v00_buildtst_VMS.txt', skip_header=6)
        if SPEC == 'WM':
            SBmodel_name = 'Z00_V00_sampling'
        if SPEC == 'FW':
            #SBmodel_name = 'Z00_V00_sampling_FW_nplanck' #SBmodel_name = 'Z00_V00_sampling_FW_VMS_spec' Z00_V00_sampling_FW_VMS
            #SBmodel_name = 'Z00_V00_sampling_FW_nplanck'
            SBmodel_name = 'Z00_V00_sampling'
    lowmass_params = np.genfromtxt('spec_params_lowmassm13_Aug.txt')
    lowmass_flux = np.genfromtxt('lcb97_m13_reform_Aug.txt')# * 12
    hires_params = np.genfromtxt('spec_params_ifa_line_m13.txt')
    ...
    if SPEC == 'WM':
        spectra_grid_file = 'galaxy/lejeune/WMbasic_OB_Z001_tst.dat'
    if SPEC == 'FW': 
        spectra_grid_file = 'FW_SB_grid_Z0.txt' #spectra_grid_file = 'FW_SB_gridSB99_Z0.txt'
        if IMF_mass_limits[-1] > 120.0:
            spectra_grid_file = 'FW_SB_grid_Z0_VMS.txt'

def calc_Nostars(c_masses, IMF_exponents, IMF_mass_limits):
    '''
    Compute normalisation constants for IMF if a multi-power law imf is used
    '''
    Aic = 1
    N_IMF_intervals = 2# '''Must be manually set until I figure out a decent way to auto distinguish between a list and an integer'''
    if N_IMF_intervals > 1:
        N_IMF_intervals = len(IMF_exponents)
        Aic = np.zeros_like(IMF_exponents)
        Aic[0] = 1

        for i in range(len(IMF_exponents)):
            i=i+1
            if i == len(IMF_exponents):
                break
            Aici = Aic[i-1] * (IMF_mass_limits[i]**(IMF_exponents[i] - IMF_exponents[i-1]))
            #print(Aici)
            Aic[i] = Aici

        sumando=[]
        for i in range(len(IMF_exponents)):
            sumandoi = ( IMF_mass_limits[i+1]**(2.0 - IMF_exponents[i]) - IMF_mass_limits[i]**(2.0 - IMF_exponents[i]) ) / (2 - IMF_exponents[i])
            sumando.append(sumandoi)
            
        summation = Aic * sumando
        sumnation = sum(summation)

    else:
        sumando = ( IMF_mass_limits[1]**(2.0 - IMF_exponents) - IMF_mass_limits[0]**(2.0 - IMF_exponents) ) / (2 - IMF_exponents)
        sumnation = Aic * sumando

    S = M_total / sumnation
    A = Aic * S   
    
    
    xmhigh = np.full_like(c_masses, 0)
    xmlow = np.full_like(c_masses, 0)
    
    for i in range(len(c_masses)):
        if c_masses[i] == min(c_masses):
            xmhigh[i] = 0.5*(c_masses[i]+c_masses[i+1])
            xmlow[i]  = c_masses[i]
        elif c_masses[i] == max(c_masses):
            xmhigh[i] = c_masses[i]
            xmlow[i]  = 0.5*(c_masses[i-1]+c_masses[i])
#        elif i == 596:
#            xmhigh[i] = c_masses[i]
#            xmlow[i]  = 0.5*(c_masses[i-1]+c_masses[i])
        else:
            xmhigh[i] = 0.5*(c_masses[i]+c_masses[i+1])
            #print(c_masses[i])
            xmlow[i] = 0.5*(c_masses[i]+c_masses[i-1])
    
    dens = []
    
    if N_IMF_intervals > 1:
        for i in range(len(c_masses)):
            densi = ( A[1] / (1 - IMF_exponents[1]) ) * ( xmhigh[i]**(1-IMF_exponents[1]) - xmlow[i]**(1-IMF_exponents[1]) )
            dens.append(densi)
    else:
        for i in range(len(c_masses)):
            densi = ( A / (1 - IMF_exponents) ) * ( xmhigh[i]**(1-IMF_exponents) - xmlow[i]**(1-IMF_exponents) )
            dens.append(densi)        
        
    N_stars = np.array(dens)
#    total_No_stars = sum(dens)
    total_mass = N_stars * c_masses
    print('Total No stars = ','{:0.4E}'.format(sum(N_stars)),'Total mass = ','{:0.4E}'.format(sum(total_mass)))
    return(N_stars, c_masses, dens, xmhigh, xmlow)
...
WN_spec_params, WN_spectra = read_spectra_grid(WN_spectra_grid_file)
WC_spec_params, WC_spectra = read_spectra_grid(WC_spectra_grid_file)

np.save('WN_spec_params.npy', WN_spec_params)
np.save('WN_spectra.npy', WN_spectra)

np.save('WC_spec_params.npy', WC_spec_params)
np.save('WC_spectra.npy', WC_spectra)

WN_spec_params_powr, WN_spectra_powr = read_spectra_grid(WN_powr_grid_file)
WC_spec_params_powr, WC_spectra_powr = read_spectra_grid(WC_powr_grid_file)

np.save('WN_spec_params_powr.npy', WN_spec_params_powr)
np.save('WN_spectra_powr.npy', WN_spectra_powr)

np.save('WC_spec_params_powr.npy', WC_spec_params_powr)
np.save('WC_spectra_powr.npy', WC_spectra_powr)
...
'''Interpolation block to get parameters for a given timestep'''
...
#For each grid point for an individual timestep assign a spectral type by finding the nearest neighbour in the 
#spectral type grid. So far the spectral type grid only includes theoretical O star spectral types from Martins+2005
#and needs to be expanded. Don't really need to assign spectral types for now.



####################################################################################################################################   

'''Block to compare with outputs from the original SB99 code'''

####################################################################################################################################

SBspectrum_name = 'spectrum'
SBfile_spectrum = SBmodel_name+'/'+SBmodel_name+'.'+SBspectrum_name
...
SBhires_spectrum = SBmodel_name+'/'+SBmodel_name+'.'+SBhires_name
...
def get_SB99_spec_neb(SB99timestep):
    SBtime_val = SBtimes[int(SB99timestep)]
    SBind_timestep = [i for i in range(len(SBtime)) if SBtime[i] < SBtime_val+10 and SBtime[i] > SBtime_val-10]
    SBspectrum_step = SBspectrum[SBind_timestep]
    SB99_logL = SBspectrum_step[:,2]
    
    return(SB99_logL)


SB00_flux = np.full_like(wave_grid, 0.0)
SB00_integrated_spectrum = 1e-30
SB00_hires_flux = np.full_like(hires_wave_grid, 0.0)

####################################################################################################################################   

'''Spectral synthesis block to make SEDs and spectra'''
...
def assign_spectra_to_grid_WR(timestep_temps_final,spec_params_reform, spec_params_teff, spec_params_logl, timestep_lums_final, timestep_H_abundances_final, spec_params_logg, timestep_masses_final, initial_masses, specsyn_cotests, specsyn_loggs, timestep_cnr, timestep_coher):
    ...
    for j in range(len(timestep_temps_final)):
        ...
        if timestep_lums_final[j] == -20.0:
            ...
            assigned_spec_logl.append(0.0)
            assigned_spec_logg.append(0.0)
            assigned_spectra.append(np.column_stack((wave_grid, SB00_flux)))
            assigned_integrated_spectra.append(SB00_integrated_spectrum)
...
def assign_spectra_to_grid_hires(timestep_temps_final,hires_params, hires_teffs, hires_loggs, timestep_lums_final, timestep_H_abundances_final, timestep_masses_final, initial_masses, specsyn_cotests, specsyn_loggs):
    ...
    for j in range(len(timestep_temps_final)):
        ...
        if timestep_lums_final[j] == -20.0:
            ...
            assigned_spec_teff.append(0.0)
            assigned_spectra.append(np.column_stack((hires_wave_grid, SB00_hires_flux)))
            assigned_integrated_spectra.append(SB00_integrated_spectrum)
            assigned_cont.append(np.column_stack((hires_wave_grid, SB00_hires_flux)))
...
'''Once we have a set of interpolated/andor/flux-scaled spectra we can just assign them to a star, sum them up by wavelength and mulitply by an IMF to get a flux out'''
...
'''Feedback block, compute ionising fluxes, wind power and equivalent widths'''
...
'''Ancillary functions, computing radius, planck function, etc'''
...
'''Main iteration of code over timestep'''
...
if plot_isochrones == True:
    ...
    #ax.scatter(np.log10(isoteff), isologL, label='SB99 100Myr', color='r', s=5)
    ax.scatter(np.log10(population_teffs[10]), population_lums[10], label='pySB 1Myr', s=2)
    #for i in range(len(population_teffs[plot_index])):
    #    ax.annotate((str(population_spectypes[plot_index][i]) + ' ' + str(population_lum_classes[plot_index][i])), (population_teffs[plot_index][i],population_lums[plot_index][i]), fontsize=8)
    #    ax.annotate((str(initial_masses[i])), (population_teffs[plot_index][i],population_lums[plot_index][i]+0.01), fontsize=8)
    plt.title(('Isochrones'))
    plt.xlabel('Teff', fontsize=12)
    plt.ylabel('Lum', fontsize=12)
    #plt.ylim(0,7)
    plt.ylim(2.0,6.4)
    ax.invert_xaxis()
...
if plot_hires_spectra == True:
    ...
    plt.style.use('default')
    ax=fig.add_subplot(111)
    #SB99_2Myr = 10**get_SB99_hires_spec(4, 'notnorm')
    #ax.plot(hires_wave_grid_SB99, 10**get_SB99_hires_spec(4, 'notnorm'), label='SB99 2Myr')
    ax.plot(hires_wave_grid, 10**(np.log10(population_hires_flux_iterations[20]+1.0e-35)+20.), label='pySB t=2Myr')
